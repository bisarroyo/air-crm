    ...
    return (
        <div className='container mx-auto p-6 space-y-6'>
            <div className='flex items-center justify-between'>
                ...
                <Button onClick={() => { form.reset(); setShowCreate(true) }} size='sm'>
                    <Plus /> New Lead
                </Button>
            </div>
                        ...
                        {referral ? (
                            <div className='space-y-3'>
                                ...
                                <p className='text-xs text-muted-foreground'>
                                    Code: <span className='font-mono font-medium text-foreground'>{referral.code}</span>
                                </p>
                            </div>
                        ) : (
                            <p className='text-sm text-muted-foreground'>No referral code assigned yet.</p>
                        )}
                    </CardContent>
                </Card>
                        ...
                        <div className='grid grid-cols-2 gap-4'>
                            ...
                            <div>
                                <p className='text-2xl font-medium'>
                                    {leads.filter(l => l.statusName?.toLowerCase() === 'new' || l.statusName?.toLowerCase() === 'nuevo').length}
                                </p>
            ...
            {showCreate && (
                <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50'>
                    <div className='mx-4 w-full max-w-md rounded-xl bg-card p-6 shadow-lg ring-1 ring-foreground/10'>
                        <h2 className='mb-1 text-lg font-medium'>Create Lead</h2>
                        <p className='mb-4 text-sm text-muted-foreground'>
                            Add a new lead. Your referral code will be automatically assigned.
                        </p>
                        <form onSubmit={form.handleSubmit(data => createMutation.mutate(data))} className='grid gap-4'>
                            <FieldGroup>
                                ...
                                <Controller name='email' control={form.control}
                                    render={({ field, fieldState }) => (
                                        <Field data-invalid={fieldState.invalid}>
                                            ...
                                            <Input {...field} type='email' placeholder='email@example.com' aria-invalid={fieldState.invalid} />
                                            {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                                        </Field>
                                    )}
                                />
                                <Controller name='phone' control={form.control}
                                    render={({ field, fieldState }) => (
                                        <Field data-invalid={fieldState.invalid}>
                                            ...
                                            <Input {...field} placeholder='Phone number' aria-invalid={fieldState.invalid} />
                                            {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                                        </Field>
                                    )}
                                />
                                <Controller name='travelTime' control={form.control}
                                    render={({ field }) => (
                                        <Field>
                            ...
                            <div className='flex justify-end gap-2'>
                                ...
                                <Button type='submit' disabled={createMutation.isPending}>
                                    {createMutation.isPending ? <Loader2 size={16} className='animate-spin' /> : 'Create Lead'}
                                </Button>
                            </div>
                        </form>
                    </div>
